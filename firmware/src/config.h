...
// Bump this on EVERY firmware change, no exceptions - shown as a persistent
// overlay on Face 0 during normal operation (standaloneRender() in
// standalone.h) so it's visible at a glance which build is actually running
