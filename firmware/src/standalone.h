...
inline const char* standaloneEffectName(uint8_t id) {
    switch (id) {
        ...
        case SA_ARTIC:         return "artic";
        case SA_UNSPLASH:      return "unsplash";
...
inline uint8_t standaloneEffectForBrowserKey(const char* key) {
    ...
    struct { const char* k; uint8_t fx; } M[] = {
        ...
        {"neo", SA_NEO}, {"artic", SA_ARTIC}, {"unsplash", SA_UNSPLASH},
...
// ===========================================================================
// Shared photo-gallery engine - faithful port of effects.js's
// galleryInitFaceState/gallerySlideshowStep/galleryApplyToFace/
// galleryApplyBlendToFace (CLAUDE.md's "shared engines" note explicitly
// calls out reusing this rather than reimplementing per effect - same
// approach as the word-cascade engine above). Used by both Art Gallery
// (Met Museum API, keyless) and Unsplash (needs a browser-supplied API key,
// synced separately below) - each face cycles on its own staggered
// schedule and crossfades into its next photo, exactly like the browser.
//
// DECODE ARCHITECTURE: JPEG download+decode is real work (network I/O +
// TJpg_Decoder), too slow to do inside the render path (which runs on the
// DMA display task at ~20fps) - so pixelsArr entries are a request/result
// state machine instead of the browser's simple array-of-cached-pixels:
//   nullptr        = not requested yet
//   GALLERY_WANTED = requested, decode not yet started
//   GALLERY_ERROR  = decode attempted and failed
//   (real pointer) = decoded PANEL_SIZE*PANEL_SIZE*3 RGB888 buffer, ready
// The render path only ever marks entries as wanted (galleryRequestLoad,
// called from gallerySlideshowStep exactly where the browser's loadFn is);
// main.cpp's loop() (core 1, never the DMA task) services one wanted entry
// per tick via standaloneGalleryDownloadJpeg - same never-block-the-display
// principle as every other network fetch in this file.
// ===========================================================================
#define GALLERY_URL_LEN 300
#define GALLERY_WANTED ((uint8_t*)1)
#define GALLERY_ERROR  ((uint8_t*)2)

struct GalleryFaceState { int curIdx; int nextIdx; float fadeT; float timer; };

inline void galleryInitFaceState(GalleryFaceState* states, int n, float periodSecs) {
    float stagger = periodSecs / 6.0f;
    for (int f = 0; f < 6; f++) {
        states[f].curIdx = n ? f % n : 0;
        states[f].nextIdx = -1;
        states[f].fadeT = 0;
        states[f].timer = f * stagger;
    }
}
inline void galleryRequestLoad(uint8_t** pixelsArr, int n, int idx) {
    if (idx < 0 || idx >= n) return;
    if (pixelsArr[idx] == nullptr) pixelsArr[idx] = GALLERY_WANTED;
}
inline void gallerySlideshowStep(GalleryFaceState& st, int n, float dt, float periodSecs, float fadeDur, bool slideshowOn, uint8_t** pixelsArr) {
    galleryRequestLoad(pixelsArr, n, st.curIdx);
    if (st.nextIdx >= 0) galleryRequestLoad(pixelsArr, n, st.nextIdx);
    if (st.fadeT > 0) {
        uint8_t* nextPixels = pixelsArr[st.nextIdx];
        if (nextPixels == GALLERY_ERROR) {
            st.nextIdx = (st.nextIdx + 1) % n;
            galleryRequestLoad(pixelsArr, n, st.nextIdx);
        } else if (nextPixels != nullptr && nextPixels != GALLERY_WANTED) {
            st.fadeT += dt;
            if (st.fadeT >= fadeDur) { st.curIdx = st.nextIdx; st.nextIdx = -1; st.fadeT = 0; }
        }
    } else if (slideshowOn) {
        st.timer += dt;
        if (st.timer >= periodSecs) {
            st.timer -= periodSecs;
...
inline bool galleryApplyToFace(uint8_t** pixelsArr, int face, int idx) {
    uint8_t* px = pixelsArr[idx];
    if (px == nullptr || px == GALLERY_WANTED || px == GALLERY_ERROR) return false;
    for (int y = 0; y < PANEL_SIZE; y++) for (int x = 0; x < PANEL_SIZE; x++) {
        int pi = (y * PANEL_SIZE + x) * 3;
        snSet(face, x, y, px[pi] / 255.0f, px[pi + 1] / 255.0f, px[pi + 2] / 255.0f);
    }
    return true;
}
inline bool galleryApplyBlendToFace(uint8_t** pixelsArr, int face, int idxA, int idxB, float alpha) {
    uint8_t* a = pixelsArr[idxA]; uint8_t* b = pixelsArr[idxB];
    bool okA = a && a != GALLERY_WANTED && a != GALLERY_ERROR;
    bool okB = b && b != GALLERY_WANTED && b != GALLERY_ERROR;
    if (!okA && !okB) return false;
    if (!okA) return galleryApplyToFace(pixelsArr, face, idxB);
    if (!okB) return galleryApplyToFace(pixelsArr, face, idxA);
    for (int y = 0; y < PANEL_SIZE; y++) for (int x = 0; x < PANEL_SIZE; x++) {
        int pi = (y * PANEL_SIZE + x) * 3;
        float r = a[pi] / 255.0f + (b[pi] / 255.0f - a[pi] / 255.0f) * alpha;
        float g = a[pi + 1] / 255.0f + (b[pi + 1] / 255.0f - a[pi + 1] / 255.0f) * alpha;
        float bl = a[pi + 2] / 255.0f + (b[pi + 2] / 255.0f - a[pi + 2] / 255.0f) * alpha;
        snSet(face, x, y, r, g, bl);
    }
    return true;
}

// Generic JPEG download+decode into a caller-provided PANEL_SIZE^2*3 RGB888
// buffer - the guts of standaloneApodFetch's image-fetch step, generalized
// to take any URL/destination instead of being hardwired to APOD's own
// globals. Shares TJpgDec with APOD; since it's one decoder instance for
// the whole firmware, the callback is repointed before and restored after.
inline uint8_t* g_galleryDecodeTarget = nullptr;
inline uint16_t g_gallerySrcW = 0, g_gallerySrcH = 0;
inline bool galleryTJpgCallback(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t* bitmap) {
    if (!g_galleryDecodeTarget || !g_gallerySrcW || !g_gallerySrcH) return false;
    for (int by = 0; by < h; by++) {
        int sy = y + by;
        if (sy >= g_gallerySrcH) continue;
        int ty = (int)((long)sy * PANEL_SIZE / g_gallerySrcH);
        if (ty >= PANEL_SIZE) continue;
        for (int bx = 0; bx < w; bx++) {
            int sx = x + bx;
            if (sx >= g_gallerySrcW) continue;
            int tx = (int)((long)sx * PANEL_SIZE / g_gallerySrcW);
            if (tx >= PANEL_SIZE) continue;
            uint16_t c565 = bitmap[by * w + bx];
            uint8_t r = ((c565 >> 11) & 0x1F) * 255 / 31;
            uint8_t g = ((c565 >> 5) & 0x3F) * 255 / 63;
            uint8_t b = (c565 & 0x1F) * 255 / 31;
            int pi = (ty * PANEL_SIZE + tx) * 3;
            g_galleryDecodeTarget[pi] = r; g_galleryDecodeTarget[pi + 1] = g; g_galleryDecodeTarget[pi + 2] = b;
        }
    }
    return true;
}
    ...
    if (http2.begin(client2, url)) {
        ...
        if (code2 == 200 && len > 0 && len < 1500000) {
            ...
            if (jpgBuf) {
                ...
                if (got == (size_t)len && TJpgDec.getJpgSize(&g_gallerySrcW, &g_gallerySrcH, jpgBuf, len)) {
                    uint8_t scale = 1;
                    while (scale < 8 && (g_gallerySrcW / (scale * 2)) >= PANEL_SIZE) scale *= 2;
                    TJpgDec.setJpgScale(scale);
                    g_gallerySrcW /= scale; g_gallerySrcH /= scale;
                    g_galleryDecodeTarget = destPixels;
                    TJpgDec.setCallback(galleryTJpgCallback);
                    memset(destPixels, 0, PANEL_SIZE * PANEL_SIZE * 3);
                    if (TJpgDec.drawJpg(0, 0, jpgBuf, len) == JDR_OK) ok = true;
                    TJpgDec.setCallback(apodTJpgCallback);   // restore - shared decoder instance
                }
                free(jpgBuf);
            }
        }
...
// ---------------------------------------------------------------------------
// Art Gallery (Metropolitan Museum of Art API - keyless, no key needed)
// ---------------------------------------------------------------------------
#define ART_MAX_WORKS 15
#define ART_CANDIDATE_CAP 60
inline char     g_articUrls[ART_MAX_WORKS][GALLERY_URL_LEN];
inline uint8_t* g_articPixels[ART_MAX_WORKS] = {nullptr};
inline int      g_articWorkCount = 0;
inline String   g_articError;
inline volatile bool g_articFetching = false;
inline GalleryFaceState g_articFaceState[6];
inline bool     g_articFaceStateInit = false;
inline float    g_articT = 0;
inline const float ART_SECS = 10.0f;
inline const float ARTIC_FADE_DUR = 1.0f;

inline bool standaloneArticFetch() {
    if (WiFi.status() != WL_CONNECTED) return false;
    if (!psramFound()) { g_articError = "PSRAM unavailable"; return false; }
    if (g_articFetching) return false;
    g_articFetching = true;
    g_articError = "";
    g_articWorkCount = 0;
    ...
    // searches the same term until that sync is added as a follow-up).
    if (http.begin(client, "https://collectionapi.metmuseum.org/public/collection/v1/search?hasImages=true&q=painting")) {
        int code = http.GET();
        if (code == 200) {
            String payload = http.getString();
            http.end();
            JsonDocument doc;
            if (!deserializeJson(doc, payload)) {
                JsonArray ids = doc["objectIDs"].as<JsonArray>();
                int total = ids.size();
                if (total > 0) {
                    int candCount = total < ART_CANDIDATE_CAP ? total : ART_CANDIDATE_CAP;
                    static int idxPool[ART_CANDIDATE_CAP];
                    for (int i = 0; i < candCount; i++) idxPool[i] = ids[i].as<int>();
                    for (int i = candCount - 1; i > 0; i--) {   // Fisher-Yates shuffle
                        int j = (int)(shRandom01() * (i + 1));
                        int tmp = idxPool[i]; idxPool[i] = idxPool[j]; idxPool[j] = tmp;
                    }
                    for (int i = 0; i < candCount && g_articWorkCount < ART_MAX_WORKS; i++) {
                        char objUrl[128];
                        ...
                        if (http2.begin(client2, objUrl)) {
                            ...
                            if (code2 == 200) {
                                ...
                                JsonDocument doc2;
                                if (!deserializeJson(doc2, p2)) {
                                    bool isPublicDomain = doc2["isPublicDomain"] | false;
                                    const char* imgUrl = doc2["primaryImageSmall"] | "";
                                    if (isPublicDomain && imgUrl[0]) {
                                        strncpy(g_articUrls[g_articWorkCount], imgUrl, GALLERY_URL_LEN - 1);
                                        g_articUrls[g_articWorkCount][GALLERY_URL_LEN - 1] = 0;
                                        g_articWorkCount++;
                                    }
                                }
                            }
                    ...
                    ok = g_articWorkCount > 0;
                    if (!ok) g_articError = "No public-domain images found";
                } else {
                    g_articError = "No results found";
                }
            } else {
                g_articError = "Parse error";
            }
        } else {
            g_articError = "Met API error " + String(code);
            http.end();
        }
    } else {
        g_articError = "Network error";
    }
    if (ok) {
        for (int i = 0; i < ART_MAX_WORKS; i++) g_articPixels[i] = nullptr;
        g_articFaceStateInit = false;
    }
    g_articFetching = false;
    return ok;
}

inline void standaloneRenderArtic(MatrixPanel_I2S_DMA* display, int face, float t) {
    (void)display;
    static float lastT = -1;
    float dt = lastT < 0 ? 0 : (t - lastT);
    lastT = t;
    if (face == 0) g_articT += dt;

    if (g_articWorkCount == 0) {
        snClear(face);
        const float amber[3] = {0.7f, 0.55f, 0.15f};
        char dots[4] = ""; int n = 1 + ((int)g_articT % 3);
        for (int i = 0; i < n; i++) dots[i] = '.'; dots[n] = 0;
        wcDrawCenteredLine(face, "ART", amber, PANEL_SIZE / 2 + 8);
        wcDrawCenteredLine(face, "GALLERY", amber, PANEL_SIZE / 2);
        wcDrawCenteredLine(face, dots, amber, PANEL_SIZE / 2 - 8);
        return;
    }
    if (g_articError.length() > 0) {
        snClear(face);
        const float red[3] = {1.0f, 0.25f, 0.25f};
        wcDrawCenteredLine(face, "API", red, PANEL_SIZE / 2 + 4);
        wcDrawCenteredLine(face, "ERROR", red, PANEL_SIZE / 2 - 4);
        return;
    }

    if (!g_articFaceStateInit) {
        galleryInitFaceState(g_articFaceState, g_articWorkCount, ART_SECS);
        g_articFaceStateInit = true;
    }
    GalleryFaceState& st = g_articFaceState[face];
    gallerySlideshowStep(st, g_articWorkCount, dt, ART_SECS, ARTIC_FADE_DUR, true, g_articPixels);

    bool shown;
    if (st.fadeT > 0 && st.nextIdx >= 0) {
        shown = galleryApplyBlendToFace(g_articPixels, face, st.curIdx, st.nextIdx, fminf(1.0f, st.fadeT / ARTIC_FADE_DUR));
    } else {
        shown = galleryApplyToFace(g_articPixels, face, st.curIdx);
    }
    if (!shown) {
        snClear(face);
        if (g_articPixels[st.curIdx] == GALLERY_ERROR) {
            const float amber2[3] = {0.6f, 0.4f, 0.1f};
            wcDrawCenteredLine(face, "NO IMAGE", amber2, PANEL_SIZE / 2);
        } else {
            const float amber[3] = {0.7f, 0.55f, 0.15f};
            char dots[4] = ""; int n = 1 + ((int)g_articT % 3);
            for (int i = 0; i < n; i++) dots[i] = '.'; dots[n] = 0;
            wcDrawCenteredLine(face, "LOADING", amber, PANEL_SIZE / 2 + 4);
            wcDrawCenteredLine(face, dots, amber, PANEL_SIZE / 2 - 4);
        }
    }
}

// ---------------------------------------------------------------------------
// Unsplash photo slideshow - needs a browser-supplied API key (Unsplash has
// no keyless demo tier, unlike NASA's DEMO_KEY). Synced from the browser
// via setOption (web_server.h) into NVS (Preferences), same persistence
// mechanism the schedule already uses.
// ---------------------------------------------------------------------------
#define UNSPLASH_MAX_PHOTOS 15
inline char     g_unsplashUrls[UNSPLASH_MAX_PHOTOS][GALLERY_URL_LEN];
inline uint8_t* g_unsplashPixels[UNSPLASH_MAX_PHOTOS] = {nullptr};
inline int      g_unsplashPhotoCount = 0;
inline String   g_unsplashError;
inline volatile bool g_unsplashFetching = false;
inline GalleryFaceState g_unsplashFaceState[6];
inline bool     g_unsplashFaceStateInit = false;
inline float    g_unsplashT = 0;
inline const float UNSPLASH_SECS = 8.0f;
inline const float UNSPLASH_FADE_DUR = 1.0f;
inline String   g_unsplashApiKey;
inline String   g_unsplashQuery = "nature";

inline void standaloneLoadUnsplashSettings() {
    g_saPrefs.begin("standalone", true);
    g_unsplashApiKey = g_saPrefs.getString("unsplashKey", "");
    g_unsplashQuery = g_saPrefs.getString("unsplashQuery", "nature");
    g_saPrefs.end();
}
inline void standaloneSaveUnsplashSettings() {
    g_saPrefs.begin("standalone", false);
    g_saPrefs.putString("unsplashKey", g_unsplashApiKey);
    g_saPrefs.putString("unsplashQuery", g_unsplashQuery);
    g_saPrefs.end();
}

inline bool standaloneUnsplashFetch() {
    if (WiFi.status() != WL_CONNECTED) return false;
    if (!psramFound()) { g_unsplashError = "PSRAM unavailable"; return false; }
    if (g_unsplashApiKey.length() == 0) { g_unsplashError = "No Unsplash API key set"; return false; }
    if (g_unsplashFetching) return false;
    g_unsplashFetching = true;
    g_unsplashError = "";
    g_unsplashPhotoCount = 0;

    char url[400];
    snprintf(url, sizeof(url), "https://api.unsplash.com/photos/random?query=%s&count=%d&client_id=%s",
             g_unsplashQuery.c_str(), UNSPLASH_MAX_PHOTOS * 2, g_unsplashApiKey.c_str());
        ...
        if (code == 200) {
            ...
            if (!deserializeJson(doc, payload)) {
                ...
                for (JsonObject p : arr) {
                    if (g_unsplashPhotoCount >= UNSPLASH_MAX_PHOTOS) break;
                    const char* regular = p["urls"]["regular"] | "";
                    if (!regular[0]) continue;
                    snprintf(g_unsplashUrls[g_unsplashPhotoCount], GALLERY_URL_LEN, "%s&w=256&h=256", regular);
                    g_unsplashPhotoCount++;
                }
                ok = g_unsplashPhotoCount > 0;
                if (!ok) g_unsplashError = "No photos found for \"" + g_unsplashQuery + "\"";
            } else {
                g_unsplashError = "Parse error";
            }
        } else if (code == 401) {
            g_unsplashError = "Invalid API key";
        } else if (code == 403) {
            g_unsplashError = "Rate limited - 50 req/hr on free tier";
        } else {
            g_unsplashError = "Unsplash error " + String(code);
        }
        http.end();
    } else {
        g_unsplashError = "Network error";
    }
    if (ok) {
        for (int i = 0; i < UNSPLASH_MAX_PHOTOS; i++) g_unsplashPixels[i] = nullptr;
        g_unsplashFaceStateInit = false;
    }
    g_unsplashFetching = false;
    return ok;
}

inline void standaloneRenderUnsplash(MatrixPanel_I2S_DMA* display, int face, float t) {
    (void)display;
    static float lastT = -1;
    float dt = lastT < 0 ? 0 : (t - lastT);
    lastT = t;
    if (face == 0) g_unsplashT += dt;

    if (g_unsplashPhotoCount == 0) {
        snClear(face);
        const float green[3] = {0.1f, 0.7f, 0.4f};
        if (g_unsplashError.length() > 0) {
            const float red[3] = {1.0f, 0.25f, 0.25f};
            wcDrawCenteredLine(face, "API", red, PANEL_SIZE / 2 + 4);
            wcDrawCenteredLine(face, "ERROR", red, PANEL_SIZE / 2 - 4);
        } else {
            char dots[4] = ""; int n = 1 + ((int)g_unsplashT % 3);
            for (int i = 0; i < n; i++) dots[i] = '.'; dots[n] = 0;
            wcDrawCenteredLine(face, "UNSPLASH", green, PANEL_SIZE / 2 + 4);
            wcDrawCenteredLine(face, dots, green, PANEL_SIZE / 2 - 4);
        }
        return;
    }

    if (!g_unsplashFaceStateInit) {
        galleryInitFaceState(g_unsplashFaceState, g_unsplashPhotoCount, UNSPLASH_SECS);
        g_unsplashFaceStateInit = true;
    }
    GalleryFaceState& st = g_unsplashFaceState[face];
    gallerySlideshowStep(st, g_unsplashPhotoCount, dt, UNSPLASH_SECS, UNSPLASH_FADE_DUR, true, g_unsplashPixels);

    bool shown;
    if (st.fadeT > 0 && st.nextIdx >= 0) {
        shown = galleryApplyBlendToFace(g_unsplashPixels, face, st.curIdx, st.nextIdx, fminf(1.0f, st.fadeT / UNSPLASH_FADE_DUR));
    } else {
        shown = galleryApplyToFace(g_unsplashPixels, face, st.curIdx);
    }
    if (!shown) {
        snClear(face);
        if (g_unsplashPixels[st.curIdx] == GALLERY_ERROR) {
            const float amber2[3] = {0.6f, 0.4f, 0.1f};
            wcDrawCenteredLine(face, "NO IMAGE", amber2, PANEL_SIZE / 2);
        } else {
            const float green[3] = {0.1f, 0.7f, 0.4f};
            char dots[4] = ""; int n = 1 + ((int)g_unsplashT % 3);
            for (int i = 0; i < n; i++) dots[i] = '.'; dots[n] = 0;
            wcDrawCenteredLine(face, "PHOTO", green, PANEL_SIZE / 2 + 4);
            wcDrawCenteredLine(face, dots, green, PANEL_SIZE / 2 - 4);
        }
    }
}
...
inline void standaloneRender(MatrixPanel_I2S_DMA* display, float dt) {
    ...
    for (uint8_t face = 0; face < NUM_FACES; face++) {
        switch (g_standaloneEffect) {
            ...
            case SA_ARTIC:         standaloneRenderArtic(display, face, t);         break;
            case SA_UNSPLASH:      standaloneRenderUnsplash(display, face, t);      break;
            default:
